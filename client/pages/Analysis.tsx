  ...
  const exportPDF = () => {
    ...
    const rowsHtml = (rows.length
      ? rows
          .map(
            (r) =>
              `<tr><td>${r.id}</td><td>${escapeHtml(r.name)}</td><td>${escapeHtml(r.type)}</td><td>${escapeHtml(maskText(r.description))}</td><td>${escapeHtml(r.description)}</td><td>${escapeHtml(r.insights)}</td></tr>`
          )
          .join("")
      : `<tr><td colspan="6" style="text-align:center;color:#64748b">No output yet.</td></tr>`);
    ...
    const doc = `<!doctype html><html><head><meta charset="utf-8"/>
      ...
      <table>
        ...
        <tfoot><tr><td colspan="6">© Optiv Security</td></tr></tfoot>
      </table>
    </body></html>`;
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Masked data</TableHead>
                  ...
                  <TableBody>
                    {rows.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={6} className="text-center text-sm text-muted-foreground">No output yet. Upload a file to begin.</TableCell>
                      </TableRow>
                    ) : (
                      rows.map((r) => (
                        <TableRow key={r.id}>
                          ...
                          <TableCell className="font-mono text-sm">{maskText(r.description)}</TableCell>
